# Sporttipaivakirja
* Käyttäjä pystyy luomaan tunnuksen ja kirjautumaan sisään sovellukseen
* Käyttäjä pystyy lisäämään sovellukseen omia urheilusuorituksia / treenejään. Lisäksi käyttäjä pystyy muokkaamaan ja poistamaan lisäämiään treenejä / urheilusuorituksia
* Käyttäjä näkee sovelluksessa omat, sekä muiden lisäämät urheilusuoritukset 
* Käyttäjä pystyy etsimään urheilusuorituksia hakusanalla tai muulla perusteella. Käyttäjä pystyy hakemaan sekä itse lisäämiään, että muiden käyttäjien lisäämiä urheilusuorituksia
* Sovelluksessa on käyttäjäsivut, jotka näyttävät käyttäjän urheilusuoritukset sekä tilastoja niistä
* Käyttäjä pystyy luokittelemaan oman suorituksensa, lajin tai muun asian perusteella
* Käyttäjä pystyy reagoimaan muiden urheilusuorituksiin sovelluksessa kommentoimalla
